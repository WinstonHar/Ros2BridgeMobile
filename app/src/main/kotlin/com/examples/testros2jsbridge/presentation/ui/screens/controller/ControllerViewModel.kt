...
class ControllerViewModel(
    ...
    fun assignButton(button: String, actionName: String) {
        val action = _appActions.value.find { it.displayName == actionName } ?: return
    ...
    fun exportConfig(outputStream: java.io.OutputStream, repository: com.examples.testros2jsbridge.data.repository.ControllerRepositoryImpl) {
        // Export the full config using repository logic
        repository.exportConfigToStream(outputStream)
    }

    fun importConfig(inputStream: java.io.InputStream, repository: com.examples.testros2jsbridge.data.repository.ControllerRepositoryImpl) {
        // Import config using repository logic
        repository.importConfigFromStream(inputStream)
        // After import, reload config
        viewModelScope.launch {
            val config = loadControllerConfigUseCase.load()
            _controllerConfig.value = config
            _controllerButtons.value = config.buttonList
            _appActions.value = config.appActions
            _presets.value = config.presets
